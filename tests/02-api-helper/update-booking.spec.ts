/**
 * tests/02-api-helper/update-booking.spec.ts
 * -----------------------------------------------------------------------------
 * LEVEL 2 - using the shared `ApiHelper`.
 *
 * PUT /booking/{id} -> full replace, with the Cookie token. We still fetch the
 * token by hand here (Level 3 hides it in a fixture), but every call now goes
 * through ApiHelper so headers/timeouts are passed the same way each time.
 */
